...
public class PatternSearch extends DirectedAcyclicGraph {
	...
	public static ETLFlowGraph applyPatterns(ETLFlowGraph G, Hashtable<Integer, ETLFlowOperation> ops){
		...
		while (graphIter.hasNext()) {
			...
			patternLinksPerNode = getLinksToPatternsForNode(G, ops, v);
			if (patternLinksPerNode.size() != 0){
				//in the future, check is there are overlappings here
				for (Pattern currentNodePattern: patternLinksPerNode){
					for(Pattern pattern: patternNodes.keySet()){
						if (currentNodePattern == pattern){
							for (ETLFlowOperation op: patternNodes.get(pattern)){
								G.removeVertex(op);
								lastPatternOperation = op;
							}
						}
						G.addVertex(currentNodePattern);
						G.addEdge(node, currentNodePattern);
						ArrayList<ETLFlowOperation> targets= getTargetNodes(G, ops, lastPatternOperation);
						for (ETLFlowOperation targetOp:targets){
						G.addEdge(currentNodePattern, targetOp);
						}
					}
	...
	public static HashMap<Integer, ArrayList<Pattern>> getAllLinksToPatterns(
			ETLFlowGraph G, Hashtable<Integer, ETLFlowOperation> ops) {
		ArrayList<String> flagNames = new ArrayList<String>();
		// In the HashMap: vector id, and an array of string pattern id's
	...
	public static HashMap<Pattern, ArrayList<ETLFlowOperation>> getNodesOfExistingPatterns(
		...
		//System.out.println("existing patterns "+existingPatterns);
		System.out.println("======================================");
		return patternNodes;	
	}
